...
from order_manager import (
    calc_qty, set_leverage, get_leverage,
...
def print_signal(signal: str, symbol: str, engine_name: str, lev: int,
                 qty: float, entry: float, tp: float, sl: float):
    color, arrow, label = (G, "^", "LONG ") if signal == "Buy" else (R, "v", "SHORT")
    print(f"\n{color}{B}+------ 신호 발생 ------------------------------+\n"
          f"  {arrow} {label}  {symbol}  {engine_name}  레버리지 {lev}x\n"
...
async def run_entry_check(wallet: float):
    ...
    for symbol in watchlist:
        try:
            ...
            df_15m = await fetch_candles(symbol, TF_ENTRY, limit=CANDLES_NEEDED)
            if len(df_15m) < CANDLES_NEEDED:
                continue
            ...
            signal = get_signal(df_15m, adx_htf, ema_htf, symbol)
            if signal is None:
                await asyncio.sleep(0.4)
                continue
...
async def main():
    ...
    while True:
        try:
            ...
            now = dt.datetime.now()
            # [수정] 15분봉 필터 하향에 따라 업데이트 주기를 1시간 -> 15분(0, 15, 30, 45분)으로 단축
            if now.minute % 15 == 0:
                await refresh_watchlist()
                await run_regime_update()
