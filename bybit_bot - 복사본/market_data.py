...
def _sync_time_offset():
    """PC 시계와 바이비트 서버 시간 오차를 계산 (lambda 패치 대신 변수로 관리)."""
    global _time_offset
    try:
        resp = _req.get("https://api.bybit.com/v5/market/time", timeout=5)
        if resp.status_code == 200:
            server_ms = int(resp.json()["result"]["timeNano"]) // 1_000_000
            local_ms = int(time.time() * 1000)
            _time_offset = (server_ms - local_ms) / 1000.0
            if abs(_time_offset) > 1:
                log.info(f"서버 시간 오차 감지: {_time_offset:.3f}초")
    except Exception as e:
        log.warning(f"시간 동기화 실패: {e}")
